...
module.exports = function (template) {
    ...
    template.AOTcompile = function (source, options) {
        ...
        // fmd 模块id resolve
        var idResolve = options.resolve || [];
        if(type.indexOf('fmd') !==-1 && idResolve.length){
            try{
                var _reg = typeof idResolve[0] === 'string' ? new RegExp(idResolve[0]) : idResolve[0];
                filename = filename.replace(_reg,idResolve[1]);
            }catch(e){
        ...
        switch (type) {
            case 'customize':
             ...
             // 完全自定义，获取配置里的执行函数
                try{
                   var cusFunc = require(options.customizePath);
                    code = cusFunc(filename,source,options);

                }catch(e){
                    code = "";
                    console.log("customize failed");
                    console.log(e);
                }
                break;
            // 纯FdmJs 模块格式,不经任何编译
            ...
            // FdmJs 模块格式
            case 'fmd':
                var _require = requires.length?",'"+ requires.join("','")+"'":'';
                code
                = "define('"+filename+"',"
                + "['" + getRuntime() +"'"+_require+"],"
                + "function(template){"
    ...
    var EXTNAME_RE = /\.(html|htm|tpl|art)$/i;
    ...
    // 依赖分析
    var parseDependencies = function (code,options) {
        var list = [];
        var uniq = {};
        ...
        .replace(INCLUDE_RE, function(m, m1, m2) {
            if (m2 && !uniq.hasOwnProperty(m2)) {
                // fmdjs 增加模块路径
                if(options.type == 'fmd'){
                    var _m2 = resolve(path.join(dirname(options.filename) , m2)).replace(/(\.\/)|(\.\.\/)/,'');
                    list.push(_m2);
                }else{
                    list.push(m2);
                }
