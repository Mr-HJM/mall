...
//                }
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<form class="edui-image-form" method="post"  target="up" enctype="multipart/form-data">' +
            '<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" name="file" type="file" hidefocus  accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            '</form>' +
        ...
        config: function (sel) {
            var me = this,
                ...
                token = me.editor.options.token;

            $('form').append('' +
                '<input style=\"filter: alpha(opacity=0);\" type="text" hidefocus name="access_token" value="'+token+'"/>');
            //url=url + (url.indexOf("?") == -1 ? "?" : "&");//初始form提交地址+ "editorid="+me.editor.id;
        ...
        uploadComplete: function(r){
            var me = this;
            try{
                var json = JSON.parse($(r).html());
                //var json = eval('('+r+')');
                Base.callback(me.editor, me.dialog, json.result.filePath, json.status);
            }catch (e){
        ...
        submit: function (callback) {
            ...
            var me = this,
                input = $( '<input style="filter: alpha(opacity=0);" class="edui-image-file" name="file" type="file" hidefocus="" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp">'),
                input = input[0];
